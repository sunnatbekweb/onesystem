...
export default function FormSection() {
    ...
    useEffect(() => {
        if (form.username.length === null) {
            ...
            setError({ ...error, username: '*Please enter a maximum of 25 characters.' })
            setCorrect({ ...correct, username: false })
        } else if (form.username.match(/\d+/)) {
            setError({ ...error, username: '*Please enter only "letters" in this field.' })
            setCorrect({ ...correct, username: false })
        } else {
            setCorrect({ ...correct, username: 'This field has been filled correctly!' })
    ...
    useEffect(() => {
        ...
        if (form.email.length === null) {
            ...
            setCorrect({ ...correct, email: 'This field has been filled correctly!' });
    ...
    const formSubmission = async (e) => {
        ...
        if (isFormValid) {
            ...
            await axios.post(`${import.meta.env.VITE_BASE_URL}/contact/`, form)
    ...
    async function sendData(username, email) {
        ...
        const response = await fetch(`https://api.telegram.org/bot${TOKEN}/sendMessage?chat_id=${botID}&text=${info}&parse_mode=html`);
        ...
        <div>
            ...
            <section className="formSection bg-[#EAEAEA] pt-[96px] pb-[224px] xl:pt-[116px] xl:pb-[198px]">
                <div className="container formSection__container w-full flex flex-col justify-center items-center px-[16px] md:px-[36px] lg:px-[48px]">
                    <form className="formSection__form w-full flex flex-col justify-center items-start gap-12 xl:w-[640px]" onSubmit={formSubmission}>
                        <div
                            className="formSection__form-input-boxes w-full flex flex-col justify-between items-start gap-3"
                            data-aos="zoom-out-down">
                            <label className="formSection__form-label font-bold text-[16px] leading-[16px] text-[#2F2F34] w-full" htmlFor="username">
                        ...
                        <div
                            className="formSection__form-input-boxes w-full flex flex-col justify-between items-start gap-3"
                            data-aos="zoom-out-down">
                            <label className="formSection__form-label font-bold text-[16px] leading-[16px] text-[#2F2F34] w-full" htmlFor="email">
                                Your email
                            </label>
                            <hr className="formSection__form-hr border-none outline-none w-full h-[2px] bg-[#D7D7D8]" />
                            <input
                        ...
                        <div
                            ...
                            data-aos="zoom-out-down">
                            <hr className="formSection__form-hr border-none outline-none w-full h-[2px] bg-[#D7D7D8]" />
                            <button className={`formSection__form-button w-full flex flex-row justify-between items-start ${form.username.length && form.email.length > 0 ? 'text-[#2F2F34]' : 'text-[#8d8d8f]'}`} type="submit">
                                <p className="formSection__form-button-text cursor-pointer font-black text-[32px] leading-[28px] tracking-tighter-[-2%] uppercase">Send</p>
                                <RightChevron fill={form.username.length && form.email.length > 0 ? '#2F2F34' : '#8d8d8f'} />
                            </button>
                        </div>
                    </form>
                </div>
